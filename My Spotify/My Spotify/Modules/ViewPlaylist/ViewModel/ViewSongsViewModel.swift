        ...
        APIManager.shared.call(type: .getPlaylistSongs(id: id)) { [weak self] (result: Result<Song, CustomError>) in
            ...
            case .success(let songs):
                ...
                let songsData = songs.tracks?.items?.map { item in
                    ...
                    return DisplaySongData(songName: item.track?.name, artistsName: artists, image: item.track?.album?.images?.first?.url)
                }
        ...
        APIManager.shared.call(type: .getAlbumTracks(id: id)) { [weak self] (result: Result<AlbumSong, CustomError>) in
            ...
            case .success(let albumSongs):
                ...
                let songData = albumSongs.tracks?.items?.map { item in
                    ...
                    return DisplaySongData(songName: item.name, artistsName: artists, image: "", id: albumSongs.id)
                }
